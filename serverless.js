...
class NextjsComponent {
  ...
  async emptyBuildDirectory(nextConfigPath) {
    return Promise.all([
      ...
      emptyDir(join(nextConfigPath, STATIC_PAGES_CODE_DIR))
    ]);
  }
  ...
  async deploy(inputs = {}) {
    ...
    [...nonDynamicHtmlPages, ...dynamicHtmlPages].map(
      page => {
        copy(
          join(nextConfigPath, ".next/serverless", page),
          join(nextConfigPath, STATIC_PAGES_CODE_DIR, page.replace("pages/", ""))
        )
      }
    );
...
const serverless = new NextjsComponent();
serverless.default()
console.log(".serverless was made for serverless.yml")